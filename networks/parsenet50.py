...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        self.c5_a = _ChannelAttention(in_planes=2048)
        ...
        # Binary Branch
        self.edge = _BoundaryAwareness(in_fea=[256, 512])
    ...
    def forward(self, x):
        ...
        c2 = self.layer1(c1)  # (-1, 256, 128, 128)
        c3 = self.layer2(c2)  # (-1, 512, 64, 64)
        c4 = self.layer3(c3)  # (-1, 1024, 32, 32)
        c5 = self.layer4(c4)  # (-1, 2048, 16, 16)
...
def FaceParseNet50(num_classes=19, pretrained=True):
...
def FaceParseNet101(num_classes=19, pretrained=True):
    model = ResNet(Bottleneck, [3, 4, 23, 3], num_classes)
    if pretrained:
        pretrained_dict = load_url("https://download.pytorch.org/models/resnet101-5d3b4d8f.pth")
        model_dict = model.state_dict().copy()
        pretrained_dict = {k: v for k,
                                    v in pretrained_dict.items() if k in model_dict}
        model_dict.update(pretrained_dict)
